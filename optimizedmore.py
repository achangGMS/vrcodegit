...
def rotate_to_best_dir():
        ...
        while (1):
            ...
            if ((not(northcount == 1000)) and (not(northblocked == WALLSBLOCKED))):
                if (northcount == numVisits):
                    drivetrain.turn_to_heading(0, DEGREES)
                    return

            # if east is not border or closed 
            if (not(eastcount == 1000) and (not(eastblocked == WALLSBLOCKED))):
                if (eastcount == numVisits):
                    drivetrain.turn_to_heading(90, DEGREES)
                    return

            # if east is border skip
            if (not(southcount == 1000) and (not(southblocked == WALLSBLOCKED))):
                if (southcount == numVisits):
                    drivetrain.turn_to_heading(180, DEGREES)
                    return

            # if east is border skip
            if (not(westcount == 1000) and (not(westblocked == WALLSBLOCKED))):
